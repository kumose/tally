...
TEST_F(PercentileTest, add) {
    tally::detail::Percentile p;
    for (int j = 0; j < 10; ++j) {
        for (int i = 0; i < 10000; ++i) {
            p << (i + 1);
        }
        tally::detail::GlobalPercentileSamples b = p.reset();
        ...
        KLOG(INFO) << "99%:" << b.get_number(0.99) << ' '
...
TEST_F(PercentileTest, merge1) {
    ...
    for (int repeat = 0; repeat < 100; ++repeat) {
        tally::detail::PercentileInterval<SAMPLE_SIZE*3> b0;
        tally::detail::PercentileInterval<SAMPLE_SIZE> b1;
        ...
        b0.merge(b1);
        tally::detail::PercentileInterval<SAMPLE_SIZE * 2> b2;
...
TEST_F(PercentileTest, merge2) {
    ...
    for (int repeat = 0; repeat < 100; ++repeat) {
        tally::detail::PercentileInterval<64> b0;
        tally::detail::PercentileInterval<64> b1;
        ...
        b0.merge(b1);
        tally::detail::PercentileInterval<64> b2;
...
TEST_F(PercentileTest, combine_of) {
    ...
    size_t total = 0;
    for (int repeat = 0; repeat < 100; ++repeat) {
        tally::detail::Percentile p[num_samplers];
        for (int i = 0; i < num_samplers; ++i) {
            for (int j = 0; j < N * (i + 1); ++j) {
                p[i] << base + i * (i + 1) * N / 2+ j;
            }
        }
        std::vector<tally::detail::GlobalPercentileSamples> result;
        result.reserve(num_samplers);
        for (int i = 0; i < num_samplers; ++i) {
            result.push_back(p[i].get_value());
        }
        tally::detail::PercentileSamples<510> g;
        g.combine_of(result.begin(), result.end());
        for (size_t i = 0; i < tally::detail::NUM_INTERVALS; ++i) {
            if (g._intervals[i] == nullptr) {
                continue;
            }
            tally::detail::PercentileInterval<510>& p = *g._intervals[i];
