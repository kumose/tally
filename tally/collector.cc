...
namespace tally {
    ...
    // A thread and a special tally to collect samples submitted.
    ...
    private:
        ...
        int64_t TURBO_CACHELINE_ALIGNED _ngrab;
        int64_t _ndrop;
        int64_t _ndump;
        std::mutex _dump_thread_mutex;
        std::condition_variable _dump_thread_cond;
        turbo::LinkNode<Collected> _dump_root;
        pthread_mutex_t _sleep_mutex;
        pthread_cond_t _sleep_cond;
    };

    Collector::Collector()
            : Reducer<Collected *, CombineCollected>(VariableAttr::collect_attr()),_last_active_cpuwide_us(turbo::Time::current_microseconds()), _created(false), _stop(false),
              _grab_thread(0), _dump_thread(0), _ngrab(0), _ndrop(0), _ndump(0) {
        ...
        if (rc != 0) {
            KLOG(ERROR) << "Fail to create Collector, " << errno;
        } else {
            _created = true;
        }
    }
...
// for limiting samples returning nullptr in speed_limit()
    static CollectorSpeedLimit g_null_speed_limit = {COLLECTOR_SAMPLING_BASE, false, 0, 0};

    void Collector::grab_thread() {
        _last_active_cpuwide_us = turbo::Time::current_microseconds();
        ...
        KCHECK_EQ(0, pthread_create(&_dump_thread, nullptr, run_dump_thread, this));

        // vars
        FuncGauge<int64_t> pending_sampled_data(
                "tally_collector_pending_samples", "help", [this]() {
                    return this->_ngrab - this->_ndump - this->_ndrop;
                });
        double busy_seconds = 0;
        FuncGauge<double> busy_seconds_var([&busy_seconds]() {
            return busy_seconds;
        });
        PerSecond<FuncGauge<double> > busy_seconds_second(
                "tally_collector_grab_thread_usage", "help", &busy_seconds_var);

        FuncGauge<int64_t> ngrab_var([this]() {
            return _ngrab;
        });
        PerSecond<FuncGauge<int64_t> > ngrab_second(
                "tally_collector_grab_second", "help", &ngrab_var);
        ...
        while (!_stop) {
            ...
            turbo::LinkNode<Collected> *head = this->reset();
            if (head) {
                turbo::LinkNode<Collected> tmp_root;
                head->InsertBeforeAsList(&tmp_root);
                head = nullptr;

                // Group samples by preprocessors.
                for (turbo::LinkNode<Collected> *p = tmp_root.next(); p != &tmp_root;) {
                    turbo::LinkNode<Collected> *saved_next = p->next();
                    p->RemoveFromList();
                ...
                for (PreprocessorMap::iterator it = prep_map.begin();
                    ...
                    for (size_t i = 0; i < list.size(); ++i) {
                        ...
                        if (_ngrab >= _ndrop + _ndump +
                                      turbo::get_flag(FLAGS_tally_collector_max_pending_samples)) {
                            ++_ndrop;
                            p->destroy();
                        } else {
                            p->InsertBefore(&root);
                ...
                if (root.next() != &root) {  // non empty
                    turbo::LinkNode<Collected> *head2 = root.next();
                    root.RemoveFromList();
                    std::unique_lock lk(_dump_thread_mutex);
            ...
            if (!_stop && abstime > now) {
                timespec abstimespec =  turbo::Duration::to_timespec(turbo::Duration::milliseconds(abstime - now));
        ...
        KCHECK_EQ(0, pthread_join(_dump_thread, nullptr));
    }
...
// Adjust speed_limit to match collected samples per second
        ...
        *last_ngrab = cur_ngrab;
        ...
        if (!sl->ever_grabbed) {
            if (sl->first_sample_real_us) {
                interval_us = turbo::Time::current_microseconds() - sl->first_sample_real_us;
                if (interval_us < 0) {
                    interval_us = 0;
                }
            } else {
            ...
            new_sampling_range = turbo::get_flag(FLAGS_tally_collector_expected_per_second)
    ...
    size_t is_collectable_before_first_time_grabbed(CollectorSpeedLimit *sl) {
        if (!sl->ever_grabbed) {
            ...
            if (before_add == 0) {
                sl->first_sample_real_us = turbo::Time::current_microseconds();
            } else if (before_add >= turbo::get_flag(FLAGS_tally_collector_expected_per_second)) {
...
// Call user's callbacks in this thread.
    void Collector::dump_thread() {
        int64_t last_ns = turbo::Time::current_nanoseconds();
        ...
        PerSecond<FuncGauge<double> > busy_seconds_second(
                "tally_collector_dump_thread_usage", "help", &busy_seconds_var);

        FuncGauge<int64_t> ndumped_var([this]() {
            return this->_ndump;
        });
        PerSecond<FuncGauge<int64_t> > ndumped_second(
                "tally_collector_dump_second", "help", &ndumped_var);
        ...
        while (!_stop) {
            ...
            turbo::LinkNode<Collected> *newhead = nullptr;
            {
                std::unique_lock lk(_dump_thread_mutex);
                while (!_stop && _dump_root.next() == &_dump_root) {
                    const int64_t now_ns = turbo::Time::current_nanoseconds();
                    busy_seconds += (now_ns - last_ns) / 1000000000.0;
                    _dump_thread_cond.wait(lk);
                    last_ns = turbo::Time::current_nanoseconds();
                }
                if (_stop) {
                    break;
                }
            ...
            KCHECK(newhead != &_dump_root);
            newhead->InsertBeforeAsList(&root);

            // Call callbacks.
            for (turbo::LinkNode<Collected> *p = root.next(); !_stop && p != &root;) {
