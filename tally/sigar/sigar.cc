                                      ...
                                      SigarProcCpu *proccpu)
{
    ...
    if (entry->value) {
        prev = (SigarProcCpu *)entry->value;
    } else {
        entry->value = malloc(sizeof(*prev));
        prev = (SigarProcCpu *)entry->value;
        SIGAR_ZERO(prev);
    }
...
void copy_cached_disk_io_into_disk_io( SigarCachedProcDiskIO *cached,  SigarProcDiskIO *proc_disk_io) {
...
void calculate_io_diff(SigarProcCumulativeDiskIO * proc_disk_io, SigarCachedProcDiskIO *cached,  sigar_uint64_t time_diff, int is_first_time) {
...
{
    sigar_cache_entry_t *entry;
    SigarCachedProcDiskIO *prev;
    SigarProcCumulativeDiskIO  cumulative_proc_disk_io;
    ...
    if (entry->value) {
        prev = (SigarCachedProcDiskIO *)entry->value;
    }
    else {
        entry->value = malloc(sizeof(*prev));
        prev = (SigarCachedProcDiskIO *)entry->value;
...
int sigar_proc_list_create(sigar_proc_list_t *proclist)
{
    ...
    proclist->data = (sigar_pid_t*)malloc(sizeof(*(proclist->data)) *
                            proclist->size);
    return SIGAR_OK;
}
...
{
    proclist->data = (sigar_pid_t *)realloc(proclist->data,
                                       ...
                                       sigar_proc_list_t *proclist)
{
    if (proclist == NULL) {
        /* internal re-use */
        if (sigar->pids == NULL) {
            sigar->pids = (sigar_proc_list_t*)malloc(sizeof(*sigar->pids));
...
int sigar_proc_args_create(sigar_proc_args_t *procargs)
{
    ...
    procargs->data = (char **)malloc(sizeof(*(procargs->data)) *
                            procargs->size);
    return SIGAR_OK;
}
...
{
    procargs->data = (char **)realloc(procargs->data,
...
int sigar_file_system_list_create(sigar_file_system_list_t *fslist)
{
    ...
    fslist->data = (sigar_file_system_t *)malloc(sizeof(*(fslist->data)) *
                          fslist->size);
    return SIGAR_OK;
}
...
{
    fslist->data = (sigar_file_system_t *)realloc(fslist->data,
                           sizeof(*(fslist->data)) *
                           (fslist->size + SIGAR_FS_MAX));
    fslist->size += SIGAR_FS_MAX;
        ...
        (ptr = strchr(fs->dev_name, ':')))
    {
        *ptr = '\0'; /* "hostname:/mount" -> "hostname" */
        ...
        status = sigar_rpc_ping(fs->dev_name,
                                SigarNetConnType::SIGAR_NETCONN_UDP,
...
int sigar_cpu_info_list_create(sigar_cpu_info_list_t *cpu_infos)
{
    ...
    cpu_infos->data = (SigarCpuInfo *)malloc(sizeof(*(cpu_infos->data)) *
                             cpu_infos->size);
    return SIGAR_OK;
}
...
{
    cpu_infos->data = (SigarCpuInfo *)realloc(cpu_infos->data,
...
int sigar_cpu_list_create(sigar_cpu_list_t *cpulist)
{
    ...
    cpulist->data = (SigarCpu *)malloc(sizeof(*(cpulist->data)) *
                           cpulist->size);
    return SIGAR_OK;
}
...
{
    cpulist->data = (SigarCpu *)realloc(cpulist->data,
                            sizeof(*(cpulist->data)) *
                            (cpulist->size + SIGAR_CPU_INFO_MAX));
    cpulist->size += SIGAR_CPU_INFO_MAX;
...
int sigar_net_route_list_create(sigar_net_route_list_t *routelist)
{
    ...
    routelist->data = (sigar_net_route_t *)malloc(sizeof(*(routelist->data)) *
                             routelist->size);
    return SIGAR_OK;
}
...
{
    routelist->data =
            (sigar_net_route_t *)realloc(routelist->data,
...
int sigar_net_interface_list_create(sigar_net_interface_list_t *iflist)
{
    ...
    iflist->data = (char **)malloc(sizeof(*(iflist->data)) *
                          iflist->size);
    return SIGAR_OK;
}
...
{
    iflist->data = (char **)realloc(iflist->data,
                           sizeof(*(iflist->data)) *
                           (iflist->size + SIGAR_NET_IFLIST_MAX));
    iflist->size += SIGAR_NET_IFLIST_MAX;
...
int sigar_net_connection_list_create(sigar_net_connection_list_t *connlist)
{
    ...
    connlist->data = (sigar_net_connection_t *)malloc(sizeof(*(connlist->data)) *
                            connlist->size);
    return SIGAR_OK;
}
...
{
    connlist->data =
            (sigar_net_connection_t *)realloc(connlist->data,
                                         ...
                                         sigar_net_connection_t *conn)
{
    ...
    if (entry->value) {
        if (conn->local_address.family == SigarNetAddress::SIGAR_AF_INET6) {
            return; /* prefer ipv4 */
        }
    }
    else {
        ...
        !sigar_cache_find(sigar->net_listen, port))
    {
        ...
        int status =
            sigar_net_stat_get(sigar, &netstat,
                               SigarNetConnType::SIGAR_NETCONN_SERVER|SigarNetConnType::SIGAR_NETCONN_TCP);
                           ...
                           sigar_net_connection_t *conn)
{
    ...
    if (conn->type == SigarNetConnType::SIGAR_NETCONN_TCP) {
    ...
    else if (conn->type == SigarNetConnType::SIGAR_NETCONN_UDP) {
        /*XXX*/
    }
                                ...
                                sigar_net_connection_t *conn)
{
    ...
    SigarNetStat *netstat = getter->netstat;
...
int sigar_tcp_curr_estab(sigar_t *sigar, sigar_tcp_t *tcp)
{
    ...
    walker.flags = SigarNetConnType::SIGAR_NETCONN_CLIENT|SigarNetConnType::SIGAR_NETCONN_TCP;
...
int sigar_arp_list_create(sigar_arp_list_t *arplist)
{
    ...
    arplist->data = (sigar_arp_t *)malloc(sizeof(*(arplist->data)) *
                           arplist->size);
    return SIGAR_OK;
}
...
{
    arplist->data = (sigar_arp_t *)realloc(arplist->data,
                            sizeof(*(arplist->data)) *
                            (arplist->size + SIGAR_ARP_LIST_MAX));
    arplist->size += SIGAR_ARP_LIST_MAX;
...
int sigar_who_list_create(sigar_who_list_t *wholist)
{
    ...
    wholist->data = (sigar_who_t *)malloc(sizeof(*(wholist->data)) *
                           wholist->size);
    return SIGAR_OK;
}
...
{
    wholist->data = (sigar_who_t *)realloc(wholist->data,
                            sizeof(*(wholist->data)) *
                            (wholist->size + SIGAR_WHO_LIST_MAX));
    wholist->size += SIGAR_WHO_LIST_MAX;
...
#define RlimitOffsets(field) \
    OffsetOf(SigarResourceLimit, field##_cur), \
    OffsetOf(SigarResourceLimit, field##_max)
...
{
    int i;
    int num = iflist->number;
    char **data = iflist->data;
    for (i=0; i<num; i++) {
                                 ...
                                 sigar_net_interface_list_t *iflist)
{
    ...
    for (;;) {
        if (!sigar->ifconf_buf || lastlen) {
            ...
            sigar->ifconf_buf = (char*)realloc(sigar->ifconf_buf, sigar->ifconf_len);
        }
