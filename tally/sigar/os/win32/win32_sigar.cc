...
#include tally/sigar/os/win32/sigar_pdh.h>
...
static int get_mem_counters(sigar_t *sigar, SigarSwap *swap, SigarMem *mem)
{
    int status;
    PERF_OBJECT_TYPE *object =
...
turbo::Status sigar_mem_get(sigar_t *sigar, SigarMem *mem)
{
    ...
    if (sigar_GlobalMemoryStatusEx) {
        ...
        if (!sigar_GlobalMemoryStatusEx(&memstat)) {
             return turbo::errno_to_status(GetLastError(), "");
        }
...
static int get_idle_cpu(sigar_t *sigar, SigarCpu *cpu,
                        DWORD idx,
...
static int sigar_cpu_perflib_get(sigar_t *sigar, SigarCpu *cpu)
{
    int status;
    PERF_INSTANCE_DEFINITION *inst;
    PERF_COUNTER_BLOCK *counter_block;
...
static int sigar_cpu_ntsys_get(sigar_t *sigar, SigarCpu *cpu)
{
    DWORD retval, num;
    int i;
    SYSTEM_PROCESSOR_PERFORMANCE_INFORMATION info[SPPI_MAX];
...
static int sigar_cpu_info_get(sigar_t *sigar, SigarCpuInfo *info)
{
    HKEY key, cpu;
    int i = 0;
    char id[MAX_PATH + 1];
