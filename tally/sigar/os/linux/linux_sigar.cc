...
//
#ifdef __linux__ || defined(__linux)
#include <dirent.h>
#include <stdio.h>
#include <stdlib.h>
#include <errno.h>
...
static int get_ram(sigar_t *sigar, SigarMem *mem)
{
    char buffer[BUFSIZ], *ptr;
    FILE *fp;
    int total = 0;
...
turbo::Status sigar_mem_get(sigar_t *sigar, SigarMem *mem)
{
    ...
    if (status != SIGAR_OK) {
        return turbo::errno_to_status(status, "");
    }
...
static void get_cpu_metrics(sigar_t *sigar, SigarCpu *cpu, char *line)
{
...
int sigar_cpu_get(sigar_t *sigar, SigarCpu *cpu)
{
    char buffer[BUFSIZ];
                           ...
                           sigar_proc_list_t *proclist)
{
    ...
    const int threadbadhack = !sigar->has_nptl;
                         ...
                         SigarDiskUsage *disk)
{
    ...
    if ((status == SIGAR_OK) && iodev) {
        ...
        double tput, util;
        SigarDiskUsage *partition_usage=NULL;
...
static int get_cpu_info(sigar_t *sigar, SigarCpuInfo *info,
                        FILE *fp)
{
    char buffer[BUFSIZ], *ptr;
...
static void get_cpuinfo_max_freq(SigarCpuInfo *cpu_info, int num)
{
    int status;
...
static void get_cpuinfo_min_freq(SigarCpuInfo *cpu_info, int num)
{
    int status;
...
static SIGAR_INLINE void convert_hex_address(SigarNetAddress *address,
                                             char *ptr, int len)
{
    if (len > HEX_ENT_LEN) {
        int i;
        ...
        address->family = SigarNetAddress::SIGAR_AF_INET6;
    }
    else {
        address->addr.in =
            (len == HEX_ENT_LEN) ? hex2int(ptr, HEX_ENT_LEN) : 0;

        address->family = SigarNetAddress::SIGAR_AF_INET;
    }
}
                         ...
                         SigarNetConnType type)
{
    ...
    SigarNetConnType flags = walker->flags;
    ...
    while ((ptr = fgets(buffer, sizeof(buffer), fp))) {
        ...
        if (!((conn.remote_port && (flags & SigarNetConnType::SIGAR_NETCONN_CLIENT)) ||
              (!conn.remote_port && (flags & SigarNetConnType::SIGAR_NETCONN_SERVER))))
        {
            continue;
        }
...
int sigar_net_connection_walk(sigar_net_connection_walker_t *walker)
{
    ...
    if (flags & SigarNetConnType::SIGAR_NETCONN_TCP) {
        status = proc_net_read(walker,
                               PROC_FS_ROOT "net/tcp",
                               SigarNetConnType::SIGAR_NETCONN_TCP);
    ...
    if (flags & SigarNetConnType::SIGAR_NETCONN_UDP) {
        status = proc_net_read(walker,
    ...
    if (flags & SigarNetConnType::SIGAR_NETCONN_RAW) {
        status = proc_net_read(walker,
                               PROC_FS_ROOT "net/raw",
                               SigarNetConnType::SIGAR_NETCONN_RAW);
                       ...
                       sigar_arp_list_t *arplist)
{
    ...
    while (fgets(buffer, sizeof(buffer), fp)) {
        ...
        if (status > 0) {
            arp->address.family = SigarNetAddress::SIGAR_AF_INET;
        }
        else if ((status = inet_pton(AF_INET6, net_addr, &arp->address.addr)) > 0) {
            arp->address.family = SigarNetAddress::SIGAR_AF_INET6;
        }
        else {
            sigar_log_printf(sigar, SIGAR_LOG_WARN,
        ...
        arp->hwaddr.family = SigarNetAddress::SIGAR_AF_LINK;
...
int sigar_proc_port_get(sigar_t *sigar, SigarNetConnType protocol,
                        unsigned long port, sigar_pid_t *pid)
{
    ...
    *pid = 0;
    ...
    status = sigar_net_connection_get(sigar, &netconn, port,
                                      SigarNetConnType::SIGAR_NETCONN_SERVER|protocol);
