...
static int sigar_swap_fs_get(sigar_t *sigar, SigarSwap *swap) /* <= 10.3 */
{
    DIR *dirp;
    struct dirent *ent;
...
static int sigar_swap_sysctl_get(sigar_t *sigar, SigarSwap *swap)
...
turbo::Status sigar_swap_get(sigar_t *sigar, SigarSwap *swap)
{
    ...
    if (getswapinfo_sysctl(kswap, 1) != SIGAR_OK) {
        if (!sigar->kmem) {
            return turbo::errno_to_status(SIGAR_EPERM_KMEM, "");
        }
...
static int get_proc_times(sigar_t *sigar, sigar_pid_t pid, SigarProcTime *time)
{
    unsigned int count;
