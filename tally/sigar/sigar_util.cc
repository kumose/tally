                          ...
                          sigar_proc_args_t *procargs)
{
    ...
    while ((len = read(fd, buffer, sizeof(buffer)-1)) > 0) {
        ...
        buf = (char*)realloc(buf, total+len+1);
...
int sigar_mem_calc_ram(sigar_t *sigar, SigarMem *mem)
{
                               ...
                               const char *dirname)
{
    ...
    if (is_dev) {
        sigar_iodev_t *iodev;
        entry->value = iodev = (sigar_iodev_t *)malloc(sizeof(*iodev));
        SIGAR_ZERO(iodev);
    ...
    for (i=0; i<fslist.number; i++) {
        ...
        if (fsp->type == SIGAR_FSTYPE_LOCAL_DISK) {
            ...
            if (SIGAR_NAME_IS_DEV(fsp->dev_name)) {
                sigar_iodev_t *iodev;
                ent->value = iodev = (sigar_iodev_t *)malloc(sizeof(*iodev));
                SIGAR_ZERO(iodev);
...
void sigar_cpu_model_adjust(sigar_t *sigar, SigarCpuInfo *info)
{
    int len, i;
...
{
    return (char *)clnt_sperrno((clnt_stat)err);
}
                                  ...
                                  unsigned long version)
{
    ...
    if (protocol == SigarNetConnType::SIGAR_NETCONN_UDP) {
        client =
            clntudp_create(&addr, program, version,
                           timeout, &sock);
    }
    else if (protocol == SigarNetConnType::SIGAR_NETCONN_TCP) {
        client =
