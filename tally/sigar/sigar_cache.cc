...
{
    sigar_cache_t *table = (sigar_cache_t *)malloc(sizeof(*table));
    table->count = 0;
    table->size = size;
    table->entries = (sigar_cache_entry_t **)malloc(ENTRIES_SIZE(size));
...
static void sigar_cache_rehash(sigar_cache_t *table)
{
    ...
    sigar_cache_entry_t **new_entries =
            (sigar_cache_entry_t **)malloc(ENTRIES_SIZE(new_size));
                                     ...
                                     sigar_uint64_t key)
{
    ...
    *ptr = entry = (sigar_cache_entry_t *)malloc(sizeof(*entry));
    entry->id = key;
    entry->value = NULL;
    entry->next = NULL;
    entry->last_access_time = sigar_time_now_millis();
