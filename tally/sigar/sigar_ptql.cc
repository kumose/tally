...
#include <tally/sigar/sigar_ptql.h>
#include <tally/sigar/sigar_os.h>
...
static int ptql_branch_list_create(ptql_branch_list_t *branches) {
    ...
    branches->data = (ptql_branch_t *) malloc(sizeof(*(branches->data)) *
                                              branches->size);
...
static int ptql_branch_list_grow(ptql_branch_list_t *branches) {
    branches->data =
            (ptql_branch_t *) realloc(branches->data,
...
static int ptql_pid_port_get(sigar_t *sigar,
    ...
    unsigned long port =
            branch->data.ui32;
    int status;
    SigarNetConnType proto =
            branch->flags == PTQL_PID_UDP_PORT ?
            SigarNetConnType::SIGAR_NETCONN_UDP : SigarNetConnType::SIGAR_NETCONN_TCP;
...
static int SIGAPI ptql_env_match(sigar_t *sigar,
    ...
    procenv.type = sigar_proc_env_t::SIGAR_PROC_ENV_KEY;
...
#define PTQL_LOOKUP_ENTRY(fname, cname, member, type) \
    (ptql_get_t)sigar_##fname##_get, \
    sigar_offsetof(Sigar##cname, member), \
    sizeof(Sigar##cname), \
...
static ptql_lookup_t PTQL_Time[] = {
        {"StartTime", PTQL_LOOKUP_ENTRY(proc_time, ProcTime, start_time, UI64)},
        {"User",      PTQL_LOOKUP_ENTRY(proc_time, ProcTime, user, UI64)},
        {"Sys",       PTQL_LOOKUP_ENTRY(proc_time, ProcTime, sys, UI64)},
        {"Total",     PTQL_LOOKUP_ENTRY(proc_time, ProcTime, total, UI64)},
        {NULL}
};

static ptql_lookup_t PTQL_Cpu[] = {
        {"StartTime", PTQL_LOOKUP_ENTRY(proc_cpu, ProcCpu, start_time, UI64)},
        {"User",      PTQL_LOOKUP_ENTRY(proc_cpu, ProcCpu, user, UI64)},
        {"Sys",       PTQL_LOOKUP_ENTRY(proc_cpu, ProcCpu, sys, UI64)},
        {"Total",     PTQL_LOOKUP_ENTRY(proc_cpu, ProcCpu, total, UI64)},
        {"Percent",   PTQL_LOOKUP_ENTRY(proc_cpu, ProcCpu, percent, DBL)},
        {NULL}
};

static ptql_lookup_t PTQL_Disk_IO[] = {
        {"BytesRead",    PTQL_LOOKUP_ENTRY(proc_disk_io, ProcDiskIO, bytes_read, UI64)},
        {"BytesWritten", PTQL_LOOKUP_ENTRY(proc_disk_io, ProcDiskIO, bytes_written, UI64)},
        {"BytesTotal",   PTQL_LOOKUP_ENTRY(proc_disk_io, ProcDiskIO, bytes_total, UI64)},
        {NULL}
};


static ptql_lookup_t PTQL_CredName[] = {
        {"User",  PTQL_LOOKUP_ENTRY(proc_cred_name, ProcCredName, user, STR)},
        {"Group", PTQL_LOOKUP_ENTRY(proc_cred_name, ProcCredName, group, STR)},
        {NULL}
};

static ptql_lookup_t PTQL_Mem[] = {
        {"Size",        PTQL_LOOKUP_ENTRY(proc_mem, ProcMem, size, UI64)},
        {"Resident",    PTQL_LOOKUP_ENTRY(proc_mem, ProcMem, resident, UI64)},
        {"Share",       PTQL_LOOKUP_ENTRY(proc_mem, ProcMem, share, UI64)},
        {"MinorFaults", PTQL_LOOKUP_ENTRY(proc_mem, ProcMem, minor_faults, UI64)},
        {"MajorFaults", PTQL_LOOKUP_ENTRY(proc_mem, ProcMem, major_faults, UI64)},
        {"PageFaults",  PTQL_LOOKUP_ENTRY(proc_mem, ProcMem, page_faults, UI64)},
        {NULL}
};

static ptql_lookup_t PTQL_Exe[] = {
        {"Name", PTQL_LOOKUP_ENTRY(proc_exe, ProcExe, name, STR)},
        {"Cwd",  PTQL_LOOKUP_ENTRY(proc_exe, ProcExe, cwd, STR)},
        {NULL}
};

static ptql_lookup_t PTQL_Cred[] = {
        {"Uid",  PTQL_LOOKUP_ENTRY(proc_cred, ProcCred, uid, UID)},
        {"Gid",  PTQL_LOOKUP_ENTRY(proc_cred, ProcCred, gid, UID)},
        {"Euid", PTQL_LOOKUP_ENTRY(proc_cred, ProcCred, euid, UID)},
        {"Egid", PTQL_LOOKUP_ENTRY(proc_cred, ProcCred, egid, UID)},
        {NULL}
};

static ptql_lookup_t PTQL_State[] = {
        {"State",     PTQL_LOOKUP_ENTRY(proc_state, ProcState, state, CHR)},
        {"Name",      PTQL_LOOKUP_ENTRY(proc_state, ProcState, name, STR)},
        {"Ppid",      PTQL_LOOKUP_ENTRY(proc_state, ProcState, ppid, PID)},
        {"Tty",       PTQL_LOOKUP_ENTRY(proc_state, ProcState, tty, UI32)},
        {"Nice",      PTQL_LOOKUP_ENTRY(proc_state, ProcState, nice, UI32)},
        {"Priority",  PTQL_LOOKUP_ENTRY(proc_state, ProcState, priority, UI32)},
        {"Threads",   PTQL_LOOKUP_ENTRY(proc_state, ProcState, threads, UI64)},
        {"Processor", PTQL_LOOKUP_ENTRY(proc_state, ProcState, processor, UI32)},
        {NULL}
};

static ptql_lookup_t PTQL_Fd[] = {
        {"Total", PTQL_LOOKUP_ENTRY(proc_fd, ProcFD, total, UI64)},
        {NULL}
};
...
SIGAR_DECLARE(int) sigar_ptql_query_create(sigar_ptql_query_t **queryp,
    ...
    int has_ref = 0;
    sigar_ptql_query_t *query =
            *queryp = (sigar_ptql_query_t *) malloc(sizeof(*query));
    ...
    *proclist = NULL;
    ...
    for (i = 0; i < query->branches.number; i++) {
        ...
        if (branch->op_flags & PTQL_OP_FLAG_PID) {
            ...
            if (*proclist == NULL) {
                *proclist = (sigar_proc_list_t *) malloc(sizeof(**proclist));
                SIGAR_ZERO(*proclist);
