...
// overhead of timers and tally is noticable.
// 
...
//
// typedef ::tally::MutexWithLatencyRecorder<pthread_mutex_t> my_mutex_t;
//                                       // ^^^
//                                       // you can use std::mutex (since c++11)
//                                       // or kthread_mutex_t (in kthread)
//
// // Define the mutex
...
//     // ^^^
//     // Or you can use KRPC_SCOPED_LOCK(mutex) to make it simple
//     ... 
...
// Specialize for pthread_mutex_t
    ...
    struct MutexConstructor<pthread_mutex_t> {
        bool operator()(pthread_mutex_t *mutex) const {
            ...
            KCHECK_EQ(0, rc) << "Fail to init pthread_mutex, " << errno;
            return rc == 0;
#else
    ...
    struct MutexDestructor<pthread_mutex_t> {
        bool operator()(pthread_mutex_t *mutex) const {
            ...
            KCHECK_EQ(0, rc) << "Fail to destroy pthread_mutex, " << errno;
            return rc == 0;
#else
    ...
    namespace detail {
        ...
        public:
            MutexWithRecorderBase(const MutexWithRecorderBase &) = delete;

            MutexWithRecorderBase &operator=(const MutexWithRecorderBase &) = delete;
        ...
        public:
            ...
            UniqueLockBase(mutex_type &mutex, std::try_to_lock_t try_to_lock)
                    : _timer(), _lock(mutex.mutex(), try_to_lock), _mutex(&mutex) {
    ...
    struct MutexWithRecorder
        ...
        explicit MutexWithRecorder(AverageGauge &recorder)
                : Base(recorder) {}
...
namespace std {
    ...
    template<typename Mutex>
    class lock_guard<tally::MutexWithRecorder<Mutex> >
            : public ::tally::detail::
            LockGuardBase<::tally::MutexWithRecorder<Mutex> > {
    public:
        typedef ::tally::detail::
        LockGuardBase<tally::MutexWithRecorder<Mutex> > Base;

        explicit lock_guard(::tally::MutexWithRecorder<Mutex> &mutex)
                : Base(mutex) {}
    };

    template<typename Mutex>
    class lock_guard<tally::MutexWithLatencyRecorder<Mutex> >
            : public ::tally::detail::
            LockGuardBase<::tally::MutexWithLatencyRecorder<Mutex> > {
    public:
        typedef ::tally::detail::
        LockGuardBase<tally::MutexWithLatencyRecorder<Mutex> > Base;

        explicit lock_guard(::tally::MutexWithLatencyRecorder<Mutex> &mutex)
                : Base(mutex) {}
    };

    template<typename Mutex>
    class unique_lock<tally::MutexWithRecorder<Mutex> >
            : public ::tally::detail::
            UniqueLockBase<::tally::MutexWithRecorder<Mutex> > {
    public:
        typedef ::tally::detail::
        UniqueLockBase<::tally::MutexWithRecorder<Mutex> > Base;
    ...
    template<typename Mutex>
    class unique_lock<tally::MutexWithLatencyRecorder<Mutex> >
            : public ::tally::detail::
            UniqueLockBase<::tally::MutexWithLatencyRecorder<Mutex> > {
    public:
        typedef ::tally::detail::
        UniqueLockBase<::tally::MutexWithLatencyRecorder<Mutex> > Base;
        typedef typename Base::mutex_type mutex_type;
